...
if __name__ == "__main__":
    try:
        ...
        print(f"      SELECT ")
        print(f"        fcr.contas_receber_id,")
        print(f"        fcr.valor,")
        print(f"        fcr.situacao,")
        print(f"        fcr.data_vencimento,")
        print(f"        dfp.descricao as forma_pagamento")
        print(f"      FROM processed.fato_contas_receber fcr")
        print(f"      LEFT JOIN processed.dim_formas_pagamento dfp ON fcr.forma_pagamento_id = dfp.forma_pagamento_id")
        print(f"      LIMIT 10;")
        print(f"")
        print(f"   3. Análises financeiras:")
        print(f"      -- Total a receber por situação")
        print(f"      SELECT situacao, COUNT(*) as qtd, SUM(valor) as total")
        print(f"      FROM processed.fato_contas_receber")
