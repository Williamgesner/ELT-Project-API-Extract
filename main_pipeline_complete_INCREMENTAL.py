# Responsável por: executar TODOS os pipelines INCREMENTAL de TODAS as empresas
# Este é o ORQUESTRADOR PRINCIPAL INCREMENTAL que executa os 6 pipelines em sequência
# MODO INCREMENTAL: Extração otimizada SEM limpeza de órfãos (RODAR DURANTE A SEMANA)
...
ORQUESTRADOR PRINCIPAL - MODO INCREMENTAL
Executa os 6 pipelines INCREMENTAL em sequência:
- Empresa 1 (main_pipeline_01_INCREMENTAL)
- Empresa 2 (main_pipeline_02_INCREMENTAL)
- Empresa 3 (main_pipeline_03_INCREMENTAL)
- Empresa 4 (main_pipeline_04_INCREMENTAL)
- Empresa 5 (main_pipeline_05_INCREMENTAL)
- Empresa 6 (main_pipeline_06_INCREMENTAL)
...
# Mapeamento de módulos de pipeline por empresa (MODO INCREMENTAL)
PIPELINE_MODULES = {
    1: 'main_pipeline_01_INCREMENTAL',
    2: 'main_pipeline_02_INCREMENTAL',
    3: 'main_pipeline_03_INCREMENTAL',
    4: 'main_pipeline_04_INCREMENTAL',
    5: 'main_pipeline_05_INCREMENTAL',
    6: 'main_pipeline_06_INCREMENTAL'
}
...
def executar_pipeline_empresa(empresa_id):
    """
    Executa o pipeline INCREMENTAL de uma empresa específica
    Retorna um dicionário com os resultados
    """
    print(f"\n{'='*70}")
    print(f"🚀 INICIANDO PIPELINE INCREMENTAL DA EMPRESA {empresa_id}")
    print(f"{'='*70}")
    ...
    try:
        ...
        module_name = f"main.pipeline_complete_INCREMENTAL.{PIPELINE_MODULES[empresa_id]}"
        ...
        print(f"\n❌ ERRO ao executar pipeline INCREMENTAL da Empresa {empresa_id}: {e}")
        import traceback
        traceback.print_exc()
...
def executar_todos_pipelines():
    """
    Executa todos os pipelines INCREMENTAL em sequência
    ...
    """
    print("\n" + "=" * 70)
    print("🌐 PIPELINE COMPLETO - TODAS AS EMPRESAS - MODO INCREMENTAL")
    print("=" * 70)
    print(f"📅 Data/Hora: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}")
    print(f"🏢 Empresas ativas: {len(EMPRESAS_ATIVAS)}")
    print(f"📋 IDs: {', '.join(str(e) for e in EMPRESAS_ATIVAS)}")
    print(f"⚡ MODO: INCREMENTAL (Extração otimizada SEM limpeza)")
    print(f"📅 PROGRAMAÇÃO: Executar A CADA 2 HORAS (durante a semana)")
    print("=" * 70)
    ...
    # =====================================================
    # 🚀 PASSO 4: EXECUTAR PIPELINES INCREMENTAL
    # =====================================================
    ...
    print(f"\n{'='*70}")
    print(f"🏁 EXECUÇÃO COMPLETA FINALIZADA - MODO INCREMENTAL")
    print(f"{'='*70}")
...
if __name__ == "__main__":
    try:
        ...
        ╔═══════════════════════════════════════════════════════════════╗
        ║                                                               ║
        ║   🌐 ORQUESTRADOR PRINCIPAL - MULTI EMPRESA - INCREMENTAL     ║
        ║                                                               ║
        ║   Este script executa TODOS os pipelines INCREMENTAL:         ║
